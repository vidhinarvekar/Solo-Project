...
expName = 'environmental_priming'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\vidna\\OneDrive\\Documents\\22-11-24 Environment Priming\\environmental_priming_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "NEP_scale" ---
    ...
    # --- Initialize components for Routine "env_prime" ---
    ...
    # --- Initialize components for Routine "desicion" ---
    OptionA = visual.ImageStim(
        win=win,
        name='OptionA', 
    ...
    for thisTrial_2 in trials_2:
        ...
        # --- Prepare to start Routine "NEP_scale" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('NEP_scale.started', globalClock.getTime(format='float'))
        question.setText(scale)
        slider.reset()
        # keep track of which components have finished
        NEP_scaleComponents = [text_2, question, slider]
        for thisComponent in NEP_scaleComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Ending Routine "NEP_scale" ---
        for thisComponent in NEP_scaleComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('NEP_scale.stopped', globalClock.getTime(format='float'))
        trials_2.addData('slider.response', slider.getRating())
        trials_2.addData('slider.rt', slider.getRT())
        # the Routine "NEP_scale" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
    ...
    trials_4 = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('environ.csv'),
    ...
    for thisTrial_4 in trials_4:
        ...
        # --- Prepare to start Routine "env_prime" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('env_prime.started', globalClock.getTime(format='float'))
        text_4.setText(env)
        slider_2.reset()
        # keep track of which components have finished
        env_primeComponents = [text_3, text_4, slider_2]
        for thisComponent in env_primeComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Ending Routine "env_prime" ---
        for thisComponent in env_primeComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('env_prime.stopped', globalClock.getTime(format='float'))
        trials_4.addData('slider_2.response', slider_2.getRating())
        trials_4.addData('slider_2.rt', slider_2.getRT())
        # the Routine "env_prime" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
    ...
    # --- Prepare to start Routine "env_decision_inst" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('env_decision_inst.started', globalClock.getTime(format='float'))
    # create starting attributes for key_resp_2
    key_resp_2.keys = []
    key_resp_2.rt = []
    _key_resp_2_allKeys = []
    # keep track of which components have finished
    env_decision_instComponents = [text_5, key_resp_2]
    for thisComponent in env_decision_instComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Ending Routine "env_decision_inst" ---
    for thisComponent in env_decision_instComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('env_decision_inst.stopped', globalClock.getTime(format='float'))
    # check responses
    ...
    thisExp.nextEntry()
    # the Routine "env_decision_inst" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    for thisTrial_5 in trials_5:
        ...
        # --- Prepare to start Routine "desicion" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('desicion.started', globalClock.getTime(format='float'))
        OptionA.setImage(image1)
        optionB.setImage(image2)
        # create starting attributes for key_resp_3
        key_resp_3.keys = []
        key_resp_3.rt = []
        _key_resp_3_allKeys = []
        # keep track of which components have finished
        desicionComponents = [OptionA, optionB, key_resp_3]
        for thisComponent in desicionComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        while continueRoutine:
            ...
            # if OptionA is starting this frame...
            if OptionA.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                OptionA.frameNStart = frameN  # exact frame index
                OptionA.tStart = t  # local t and not account for scr refresh
                OptionA.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(OptionA, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'OptionA.started')
                # update status
                OptionA.status = STARTED
                OptionA.setAutoDraw(True)
            
            # if OptionA is active this frame...
            if OptionA.status == STARTED:
                # update params
                pass
            
            # *optionB* updates
            
            # if optionB is starting this frame...
            if optionB.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                optionB.frameNStart = frameN  # exact frame index
                optionB.tStart = t  # local t and not account for scr refresh
                optionB.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(optionB, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'optionB.started')
                # update status
                optionB.status = STARTED
                optionB.setAutoDraw(True)
            
            # if optionB is active this frame...
            if optionB.status == STARTED:
                # update params
                pass
        ...
        # --- Ending Routine "desicion" ---
        for thisComponent in desicionComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('desicion.stopped', globalClock.getTime(format='float'))
        # check responses
        ...
        # the Routine "desicion" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
